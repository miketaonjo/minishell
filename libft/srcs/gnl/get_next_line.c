...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/04/15 13:20:10 by clorcery          #+#    #+#             */
/*   Updated: 2022/04/19 11:59:32 by clorcery         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*new_buffer(char *old_buf)
{
	...
	while (old_buf[i] != '\0' && old_buf[i] != '\n')
		i++;
	if (old_buf[i] == '\n')
		new_buf = ft_substr(old_buf, i + 1, ft_strlen(old_buf + i + 1));
	else
		new_buf = ft_substr(old_buf, i, 1);
	if (*new_buf == '\0')
	{
		free(new_buf);
		new_buf = NULL;
